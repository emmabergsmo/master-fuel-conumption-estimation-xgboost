...
def plot_normal_overlay(group: pd.DataFrame, pair_name: str, outdir: Path):
    ...
    if len(real) < 2 or len(pred) < 2:
        print(f"Skipping overlay plot for {pair_name}: not enough data points.")
        return
    ...
    if real_std == 0 or pred_std == 0:
        print(f"Skipping overlay plot for {pair_name}: zero variance in one distribution.")
        return
    ...
    xmin = max(0, xmin)  # fuel should not be negative
    ...
    outpath = outdir / f"{make_safe_name(pair_name)}_fuel_distribution.png"
...
def plot_delta_distribution(group: pd.DataFrame, pair_name: str, outdir: Path):
    """
    Plot distribution of prediction error:
    delta = predicted_total_fuel - real_total_fuel
    """
    delta = group["delta_fuel"].dropna().to_numpy()

    if len(delta) < 2:
        print(f"Skipping delta plot for {pair_name}: not enough data points.")
        return
    ...
    # Histogram
    plt.hist(delta, bins=30, density=True, alpha=0.5, label="Delta histogram")

    # Only plot normal curve if variance is non-zero
    if delta_std > 0:
        xmin = delta_mean - 4 * delta_std
        xmax = delta_mean + 4 * delta_std
        x = np.linspace(xmin, xmax, 1000)
        delta_pdf = norm.pdf(x, loc=delta_mean, scale=delta_std)
        plt.plot(x, delta_pdf, linewidth=2, label="Normal fit", alpha=0.8)

    # Perfect prediction line
    plt.axvline(0, linestyle="--", linewidth=2, label="Perfect prediction (0)")

    plt.title(f"Prediction error distribution: {pair_name}")
    plt.xlabel("Predicted fuel - Real fuel")
    plt.ylabel("Density")
    plt.xlim(-400,400)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    outpath = outdir / f"{make_safe_name(pair_name)}_delta_distribution.png"
    plt.savefig(outpath, dpi=150, bbox_inches="tight")
    plt.close()
...
# Plot output folders
# -----------------------------
overlay_outdir = BASE_DIR / "fuel_distribution_plots"
delta_outdir = BASE_DIR / "delta_distribution_plots"

overlay_outdir.mkdir(exist_ok=True)
...
# -----------------------------
plot_normal_overlay(merged, "ALL_ROUTES", overlay_outdir)
plot_delta_distribution(merged, "ALL_ROUTES", delta_outdir)
...
for pair_name, group in merged.groupby("dep_arr_pair"):
    ...
    plot_normal_overlay(group, pair_name, overlay_outdir)
    plot_delta_distribution(group, pair_name, delta_outdir)
